...
def rapidez_media():
    ...
    distancia = integrar_trapecio(velocidad_absoluta, t1, t2)
    duracion = t2 - t1
    r_med = distancia / duracion

    return {
        "formula": "Rapidez media = distancia recorrida dividida entre duración del intervalo.",
        "resultado": f"{formatear_numero(r_med)} unidades/s",
        "pasos": [
            f"Función usada: {entrada}",
            f"Intervalo: desde t1={formatear_numero(t1)} hasta t2={formatear_numero(t2)}.",
            f"Distancia aproximada: {formatear_numero(distancia)} unidades.",
            f"Duración: {formatear_numero(duracion)} unidades de tiempo.",
            f"Rapidez media: {formatear_numero(r_med)} unidades/s.",
        ],
        "interpretacion": "Es la rapidez constante que produciría el mismo recorrido total en el mismo tiempo.",
    }
